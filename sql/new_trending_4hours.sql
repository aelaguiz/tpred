...
SELECT twitter.id, twitter.topic, twitter.val as twval, hn.val, reddit.val FROM (
	...
	WHERE moment_to IN (
		SELECT DISTINCT moment FROM topic_moment ORDER BY moment DESC LIMIT 16 OFFSET 1) AND 
		site_id=1 
	GROUP BY t.id 
...
(SELECT twitter.id FROM (
	...
	JOIN topic t on tmd.topic_id=t.id 
	WHERE moment_to IN (
		SELECT DISTINCT moment FROM topic_moment ORDER BY moment DESC LIMIT 16 OFFSET 17) AND 
		site_id=1 
	GROUP BY t.id 
	ORDER BY avg(value) desc) as twitter
LEFT JOIN
