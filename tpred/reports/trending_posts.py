...
def run_report(minutes, filter_links=False):
    q = """
    SELECT original.site_id, original.body_id, COUNT(*) as num_reposts, original.body, original.sn, MAX(original.diff), original.url, original.pic_url
    FROM (
    ...
    used = set()
    for site_id, body_id, num_reposts, body, sn, diff, url, pic_url in res:
        if body_id in used:
            continue
